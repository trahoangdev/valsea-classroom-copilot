...
function GeneratingSkeleton() {
  return (
    <div className="space-y-4 px-6 py-6" aria-busy="true" aria-label="Đang tạo ghi chú">
    ...
    <Card className="flex h-full min-h-[420px] flex-col gap-0 py-0">
      <CardHeader className="flex flex-row flex-wrap items-start justify-between gap-4 border-b px-6 py-4">
        <div>
          ...
          <CardDescription>Tóm tắt, thuật ngữ và quiz từ LLM (qua gateway)</CardDescription>
        </div>
        {output ? (
        ...
        {generating ? (
          ...
          <div className="flex flex-col items-center justify-center gap-4 px-6 py-16 text-center">
            ...
            <div className="max-w-sm space-y-2">
              <p className="font-semibold text-foreground">Chưa có ghi chú</p>
              <p className="text-sm text-muted-foreground">
                Khi đã có transcript, nhấn <span className="font-medium text-foreground">Tạo ghi chú</span>{" "}
                (hoặc dùng kịch bản demo §17).
              </p>
            </div>
          </div>
        ) : (
          <ScrollArea className="h-[min(60vh,520px)] px-6 py-4">
            <div className="space-y-4 pr-3">
              ...
              {output.simpleExplanationVi ? (
                <SectionCard title="Giải thích đơn giản">
              ...
              {output.possibleConfusingPoints.length ? (
                <SectionCard title="Có thể gây nhầm lẫn">
